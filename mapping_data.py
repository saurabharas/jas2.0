...
def create_mapping_df(excel_mapping):
    ...
    # print(df_excel)
    df_excel.to_sql(
        "mapping_data_jas", engine, if_exists="replace"
...
def index_mapping(excel_mapping):
    ...
    df_excel.rename(
        {"JAS Token": "jas_token", "Index Name": "index_name"}, axis=1, inplace=True
    )

    print(df_excel)
    df_excel.to_sql(
        "mapping_data_jas_index", engine, if_exists="replace"
    )  ## Uncomment to write to the database
    db_conn.closeConn()
...
def update_nifty_500():
    # df_mapping_update = pd.read_excel(
    #     "excel_data_input/momentum_2022-04-12 with Additions & Subtractions.xlsx",
    #     sheet_name="ToBeAdded",
    # )

    ## Update Nifty500 to yes with NSE symbol
    # symbol_to_be_added = tuple(list(df_mapping_update["Symbol"]))
    # engine = create_engine(conn_string_pd)
    # query_update = f"UPDATE mapping_data_jas SET nifty_500='Yes' where nse_symbol in {symbol_to_be_added};"
    # engine.execute(query_update)

    ## Remove from Nifty500 with NSE symbol
    # df_mapping_update = pd.read_excel(
    #     "excel_data_input/momentum_2022-04-12 with Additions & Subtractions.xlsx",
    #     sheet_name="ToBeRemoved",
    # )
    # symbol_to_be_deleted = tuple(list(df_mapping_update["Symbol"]))
    # engine = create_engine(conn_string_pd)
    # query_update = f"UPDATE mapping_data_jas SET nifty_500='No' where nse_symbol in {symbol_to_be_deleted};"
    # engine.execute(query_update)

    ## Add new stocks in DB
    # df_mapping_update = pd.read_excel(
    #     "excel_data_input/momentum_2022-04-12 with Additions & Subtractions.xlsx",
    #     sheet_name="ToBeAddedNew",
    # )
    # df_mapping_update.rename(
    #     {
    #         "JAS Token": "jas_token",
    #         "Company Name": "name",
    #         "Industry": "industry",
    #         "Symbol": "nse_symbol",
    #         "ISIN Code": "isin_code_equity",
    #     },
    #     axis=1,
    #     inplace=True,
    # )
    # print(df_mapping_update, flush=True)
    # df_mapping_update.to_sql(
    #     "mapping_data_jas", engine, if_exists="append"
    # )  ## Uncomment to write to the database
    # db_conn.closeConn()
