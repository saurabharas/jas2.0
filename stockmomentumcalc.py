...
# libraries to be imported mail sending
import smtplib
...
def mail_send(workBookName, file_path):
    ...
    # storing the subject
    msg["Subject"] = "Automated Momentum Results Excel {0}".format(
        datetime.now().strftime("%Y_%m_%d")
    )

    # string to store the body of the mail
    body = "PFA the excel for momentum results."
    ...
    p.add_header("Content-Disposition", "attachment; filename= %s" % workBookName)
...
def atr_loop(df):
    ...
    for index, val in df.iterrows():
        ...
        if atr_count >= 20:
            ...
            df["atr"].iloc[atr_count] = ((previous_atr_val * 19) + current_tr_val) / 20
...
def main_stock_momemtum_calc_jas():
    ...
    # query_mapping = "Select jas_token, nse_symbol, name, mcap_crore from mapping_data_jas WHERE jas_sector_final='Healthcare' and nifty_500='Yes' ORDER BY jas_token"

    query_mapping = "Select jas_token, nse_symbol, name, mcap_crore,jas_sector_final from mapping_data_jas WHERE nifty_500='Yes' ORDER BY jas_token"
    ...
    for key, value in tokens_df.iterrows():
        try:
            if value["jas_token"] >= 0:
                ...
                jas_sector = value["jas_sector_final"]
                query_nse_data = "Select * from nse_data WHERE jas_token={0} ORDER BY timestamp_date".format(
                    jas_token
                )
                ...
                # jobj_stock_momentum["jas_sector"] = jas_sector
                ...
                print(
                    "running done for {0} -----  nse ------ {1} ".format(
                        value["jas_token"], jobj_stock_momentum["timestamp_string"]
                    ),
                    flush=True,
                )
    ...
    try:
        ...
        print(df_jobj_momentum, flush=True)
        df_jobj_momentum = df_jobj_momentum.sort_values(
